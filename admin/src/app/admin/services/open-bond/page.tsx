...
// Service data uses simplified structure (no styling controls)
...
const createDefaultData = (): ServiceData => ({
  ...
  description_en: 'Open bond service. Public bond issuance service with market trading capability.\n\nThis service will introduce you to the necessary documents and conditions.',
  stats: {
    interest: '8% - 12%',
    decision: '7-14 хоног',
    term: '1-5 жил',
  ...
  materials: [
    { id: crypto.randomUUID(), mn: 'Компанийн санхүүгийн тайлан (сүүлийн 3 жил)', en: 'Company financial report (last 3 years)' },
    { id: crypto.randomUUID(), mn: 'Бизнес төлөвлөгөө', en: 'Business plan' },
    { id: crypto.randomUUID(), mn: 'Аудитын дүгнэлт', en: 'Audit report' },
    { id: crypto.randomUUID(), mn: 'Компанийн дүрэм', en: 'Company charter' },
    { id: crypto.randomUUID(), mn: 'Үнэт цаасны хорооны зөвшөөрөл', en: 'Securities Exchange approval' },
    { id: crypto.randomUUID(), mn: 'Татварын төлбөрийн баримт', en: 'Tax payment certificate' },
  ],
  collateral: [],
  conditions: [
    { id: crypto.randomUUID(), mn: 'Компани үйл ажиллагаа явуулж 3 жилээс доошгүй хугацаа өнгөрсөн байх', en: 'Company must be operating for at least 3 years' },
    ...
    { id: crypto.randomUUID(), mn: 'Зах зээлийн хөгжлийн хорооны шаардлага хангасан байх', en: 'Must meet market development board requirements' },
  ],
  blocks: [],
  status: 'draft',
})

// Simple ServicePage preview component
function ServicePreview({ data, lang }: { data: ServiceData; lang: 'mn' | 'en' }) {
  const name = lang === 'mn' ? data.name_mn : data.name_en
  const category = lang === 'mn' ? data.category_mn : data.category_en
  ...
  return (
    <main className="min-h-screen bg-linear-to-b from-slate-50 via-white to-slate-100 text-slate-900 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 py-20 md:py-24 relative z-10">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <section className="space-y-8">
            <div className="relative bg-white rounded-4xl p-10 md:p-16 shadow-[0_20px_60px_rgba(0,0,0,0.08)] border border-slate-100">
              {/* Materials */}
...
export default function OpenBondServiceAdminPage() {
    ...
    async (mode: 'auto' | 'manual') => {
      try {
        ...
        const response = await fetch('/api/admin/services', {
          ...
          body: JSON.stringify({ ...data, type: 'open-bond', mode }),
        })
  ...
  return (
    <AdminLayout title="Нээлттэй бонд">
      <div className="max-w-6xl mx-auto">
        {saveSuccess && (
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input
                  label="Нэр (МН) *"
                  placeholder="Жишээ: Нээлттэй бонд"
                  value={data.name_mn}
                  helper="Үйлчилгээний гол нэр - сайтаас үзэгдэх"
                  onChange={(e) => updateData((prev) => ({ ...prev, name_mn: e.target.value }))}
                />
                <Input
                  label="Name (EN) *"
                  placeholder="e.g., Open Bond"
                  value={data.name_en}
                  helper="Main service name shown on website"
                  onChange={(e) => updateData((prev) => ({ ...prev, name_en: e.target.value }))}
                />
                
                <Input
                  label="Ангилал (МН) *"
                  placeholder="Жишээ: Үйлчилгээ · Бонд"
                  value={data.category_mn}
                  helper="Үйлчилгээний төрөл - навигейшнэд харагдана"
                  onChange={(e) => updateData((prev) => ({ ...prev, category_mn: e.target.value }))}
                />
                <Input
                  label="Category (EN) *"
                  placeholder="e.g., Service · Bond"
                  value={data.category_en}
                  helper="Service category for navigation"
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Input
                  label="Хүүгийн хувь / Interest Rate *"
                  placeholder="Жишээ: 8% - 12% per annum"
                  value={data.stats.interest}
                  helper="Жилийн хүүгийн хувь / Annual interest"
                  onChange={(e) => updateData((prev) => ({ ...prev, stats: { ...prev.stats, interest: e.target.value } }))}
                />
                <Input
                  label="Шийдвэрийн хугацаа / Decision Time *"
                  placeholder="Жишээ: 7-14 хоног"
                  value={data.stats.decision}
                  helper="Хүсэлтэд хариу ирэх хугацаа / Decision time"
                  onChange={(e) => updateData((prev) => ({ ...prev, stats: { ...prev.stats, decision: e.target.value } }))}
                />
                <Input
                  label="Хамгийн урт хугацаа / Max Term *"
                  placeholder="Жишээ: 1-5 жил"
                  value={data.stats.term}
                  helper="Зээлийн хамгийн их хугацаа / Max duration"
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {(data.blocks || []).length === 0 ? (
                <p className="text-sm text-gray-500 italic text-center py-8">Бүтээн байгуулалтын блок байхгүй. Шинэ блок нэмэхийн тулд дээрх товчлуур дээр дарна уу.</p>
              ) : (
                (data.blocks || []).map((block, idx) => (
                  <div key={block.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-3 relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pr-8">
                      <Input
                        label="Гарчиг (МН)"
                        value={block.title_mn}
                        onChange={(e) => {
                          const updated = [...(data.blocks || [])]
                          updated[idx].title_mn = e.target.value
                          updateData((prev) => ({ ...prev, blocks: updated }))
                        }}
                      />
                      <Input
                        label="Title (EN)"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <Textarea
                        ...
                        onChange={(e) => {
                          const updated = [...(data.blocks || [])]
                          updated[idx].content_mn = e.target.value
                          updateData((prev) => ({ ...prev, blocks: updated }))
                        }}
                        rows={3}
                      />
                      <Textarea
                        ...
                        onChange={(e) => {
                          const updated = [...(data.blocks || [])]
                          updated[idx].content_en = e.target.value
                          updateData((prev) => ({ ...prev, blocks: updated }))
                        }}
                        rows={3}
                      />
                    ...
                    <div className="pt-3 border-t border-gray-300">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const updated = [...(data.blocks || [])]
                          updated[idx].placement = e.target.value as 'section_1' | 'section_2' | 'section_3' | 'section_4'
                          updateData((prev) => ({ ...prev, blocks: updated }))
                        }}
                    ...
                    {/* Title Styling */}
                    <div className="pt-3 border-t border-gray-300">
                      <p className="text-xs font-semibold text-gray-700 mb-2">Гарчигийн стиль / Title Style</p>
                      <div className="grid grid-cols-2 gap-2">
                        <select
                          value={block.title_style?.fontSize || ''}
                          onChange={(e) => {
                            const updated = [...(data.blocks || [])]
                            updated[idx].title_style = { ...updated[idx].title_style, fontSize: e.target.value }
                            updateData((prev) => ({ ...prev, blocks: updated }))
                          }}
                        ...
                        >
                          ...
                          <option value="text-base">Байндаа (BASE)</option>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const updated = [...(data.blocks || [])]
                            updated[idx].title_style = { ...updated[idx].title_style, fontColor: e.target.value }
                            updateData((prev) => ({ ...prev, blocks: updated }))
                          }}
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        <select
                          ...
                          onChange={(e) => {
                            const updated = [...(data.blocks || [])]
                            updated[idx].title_style = { ...updated[idx].title_style, fontFamily: e.target.value }
                            updateData((prev) => ({ ...prev, blocks: updated }))
                          }}
                          className="px-2 py-1 text-xs border border-gray-300 rounded"
                        >
                          <option value="">Фонт сонгох</option>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            const updated = [...(data.blocks || [])]
                            updated[idx].title_style = { ...updated[idx].title_style, fontWeight: e.target.value }
                            updateData((prev) => ({ ...prev, blocks: updated }))
                          }}
                          className="px-2 py-1 text-xs border border-gray-300 rounded"
                        >
                          <option value="">Жинг сонгох</option>
                          <option value="400">Ердийн</option>
                          <option value="600">Хагас сүүл</option>
                          <option value="700">Сүүл</option>
                          <option value="800">Маш сүүл</option>
                        </select>
                      </div>
                    </div>
                    ...
                    {/* Content Styling */}
                    <div className="pt-3 border-t border-gray-300">
                      <p className="text-xs font-semibold text-gray-700 mb-2">Агуулгын стиль / Content Style</p>
                      <div className="grid grid-cols-2 gap-2">
                        <select
                          value={block.content_style?.fontSize || ''}
                          onChange={(e) => {
                            const updated = [...(data.blocks || [])]
                            updated[idx].content_style = { ...updated[idx].content_style, fontSize: e.target.value }
                            updateData((prev) => ({ ...prev, blocks: updated }))
                          }}
                          className="px-2 py-1 text-xs border border-gray-300 rounded"
                        >
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const updated = [...(data.blocks || [])]
                            updated[idx].content_style = { ...updated[idx].content_style, fontColor: e.target.value }
                            updateData((prev) => ({ ...prev, blocks: updated }))
                          }}
                          className="w-full h-8 border border-gray-300 rounded"
                          title="Өнгө сонгох"
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        <select
                          ...
                          onChange={(e) => {
                            const updated = [...(data.blocks || [])]
                            updated[idx].content_style = { ...updated[idx].content_style, fontFamily: e.target.value }
                            updateData((prev) => ({ ...prev, blocks: updated }))
                          }}
                          className="px-2 py-1 text-xs border border-gray-300 rounded"
                        >
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            const updated = [...(data.blocks || [])]
                            updated[idx].content_style = { ...updated[idx].content_style, fontWeight: e.target.value }
                            updateData((prev) => ({ ...prev, blocks: updated }))
                          }}
                          className="px-2 py-1 text-xs border border-gray-300 rounded"
                        >
